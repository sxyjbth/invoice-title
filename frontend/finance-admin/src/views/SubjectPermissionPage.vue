    ...
    <section class="permission-config-layout" aria-label="主体权限配置">
      ...
      <section v-if="activePermissionProfile" class="permission-detail-card">
        ...
        <div class="permission-level-row">
          ...
          <el-switch :model-value="activePermissionProfile.allEmployeesVisible" aria-label="全员可见" @update:model-value="updateAllEmployeesVisibility" />
        </div>
        ...
        <footer><p>权限保存后实时生效，员工调整后按钉钉通讯录自动更新。</p><el-button type="primary" size="large" :loading="permissionSaving" @click="savePermissionConfiguration">保存权限</el-button></footer>
      </section>
